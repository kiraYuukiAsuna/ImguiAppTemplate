...
void ed::EditorContext::SetNodePosition(NodeId nodeId, const ImVec2& position)
{
    ...
    if (node->m_Bounds.Min != position)
    {
        ...
        node->m_Bounds.Min = ImTrunc(node->m_Bounds.Min);
        node->m_Bounds.Max = ImTrunc(node->m_Bounds.Max);
...
void ed::EditorContext::SetGroupSize(NodeId nodeId, const ImVec2& size)
{
    ...
    if (node->m_GroupBounds.GetSize() != size)
    {
        ...
        node->m_GroupBounds.Min = ImTrunc(node->m_GroupBounds.Min);
        node->m_GroupBounds.Max = ImTrunc(node->m_GroupBounds.Max);
...
bool ed::SizeAction::Process(const Control& control)
{
    ...
    if (control.ActiveNode == m_SizedNode)
    {
        ...
        newBounds.Min = ImTrunc(newBounds.Min);
        newBounds.Max = ImTrunc(newBounds.Max);
...
void ed::NodeBuilder::End()
{
    ...
    m_NodeRect.Min = ImTrunc(m_NodeRect.Min);
    m_NodeRect.Max = ImTrunc(m_NodeRect.Max);
...
void ed::NodeBuilder::PinRect(const ImVec2& a, const ImVec2& b)
{
    ...
    m_CurrentPin->m_Bounds.Min = ImTrunc(m_CurrentPin->m_Bounds.Min);
    m_CurrentPin->m_Bounds.Max = ImTrunc(m_CurrentPin->m_Bounds.Max);
...
void ed::NodeBuilder::Group(const ImVec2& size)
{
    ...
    m_GroupBounds.Min = ImTrunc(m_GroupBounds.Min);
    m_GroupBounds.Max = ImTrunc(m_GroupBounds.Max);
}
